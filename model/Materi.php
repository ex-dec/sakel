...
class Materi
{
    ...
    {
        $stmt = $this->db->query("SELECT materi.* , mapel.name AS mapel_name 
                                  FROM materi JOIN mapel ON materi.mapel_id = mapel.id");
    ...
    {
        $stmt = $this->db->prepare("INSERT INTO materi (name, link, description, mapel_id) 
                                    VALUES (?, ?, ?, ?)");
